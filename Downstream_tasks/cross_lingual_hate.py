# ============================================================
# Normal End-to-End Fine-tuning for Hate Speech Detection
# ============================================================
...
# Full hate dataset
HATE_CSV  = Path("/data/hate_translated.csv")

# Split output paths — if these exist, skip re-splitting
TRAIN_CSV = Path("data-path/splits/hate_train.csv")
VAL_CSV   = Path("data-path/splits/hate_val.csv")
TEST_CSV  = Path("data-path/splits/hate_test.csv")

# CHANGED: all outputs now go to one absolute folder
OUT_ROOT   = Path("output-path/hate_results")
EXCEL_PATH = OUT_ROOT / "results_hate_hing_trans.xlsx"
# Checkpoint directory — stores progress so a crash doesn't lose everything
CKPT_DIR   = OUT_ROOT / "checkpoints_trans"
...
NUM_WORKERS = 2
# FIX 1: persistent_workers=True crashes when num_workers=0
...
def make_dataloader(enc, labels, batch_size, shuffle=False):
    ...
    return DataLoader(
        ...
        persistent_workers=persistent_workers  # safe: False when num_workers=0
    )
...
def load_all_checkpointed_results():
    ...
    FIX 6: deduplicate by (model, train_on, epoch) to prevent double-counting on resume.
    """
    all_results = []
    seen        = set()
    for p in CKPT_DIR.glob("*.json"):
        ...
        for r in data.get("results", []):
            key = (r["model"], r["train_on"], r["epoch"])
            if key not in seen:
                seen.add(key)
...
# ============================================================
# ADDED: evaluation helper  (unchanged logic, + autocast + non_blocking)
...
# ============================================================
# SPLIT DATA — 80:10:10
# If splits already exist on disk, load them directly.
# ============================================================

if TRAIN_CSV.exists() and VAL_CSV.exists() and TEST_CSV.exists():
    print("Split files already exist — loading from disk, skipping re-split.")
    train_df = pd.read_csv(TRAIN_CSV)
    val_df   = pd.read_csv(VAL_CSV)
    test_df  = pd.read_csv(TEST_CSV)
else:
    print("Split files not found — creating 80:10:10 split...")
    full_df = pd.read_csv(HATE_CSV)

    # 80 / 20 first, then split the 20 into 10/10
    train_df, temp_df = train_test_split(
        full_df, test_size=0.20, random_state=RANDOM_STATE, stratify=full_df["label"]
    )
...
train_df = pd.read_csv(TRAIN_CSV)[["sentence", "translation_en", "translation_hi", "label"]].copy()
val_df   = pd.read_csv(VAL_CSV)[["sentence", "translation_en", "translation_hi", "label"]].copy()
test_df  = pd.read_csv(TEST_CSV)[["sentence", "translation_en", "translation_hi", "label"]].copy()

train_df = train_df.rename(columns={"label": "answer"})
val_df   = val_df.rename(columns={"label": "answer"})
test_df  = test_df.rename(columns={"label": "answer"})
...
print(f"Train label dist:\n{train_df['answer'].value_counts().to_string()}")
...
for model_tag, hf_name in MODELS.items():
    ...
    for col in TEXT_COLS:
        # FIX 2: explicit max_length=128 — avoids OOM, speeds up training
        # hate speech sentences are short, 128 tokens covers >99% of them
        all_train_encs[model_tag][col] = tokenizer(
            train_df[col].tolist(), padding="max_length", truncation=True, max_length=128, return_tensors="pt"
        )
        all_val_encs[model_tag][col] = tokenizer(
            val_df[col].tolist(), padding="max_length", truncation=True, max_length=128, return_tensors="pt"
        )
        all_test_encs[model_tag][col] = tokenizer(
            test_df[col].tolist(), padding="max_length", truncation=True, max_length=128, return_tensors="pt"
        )
...
for train_col in TEXT_COLS:
    ...
    for model_tag, hf_name in MODELS.items():
        ...
        # FIX 4: torch.compile disabled — slows down BERT/RoBERTa on most clusters
        # Uncomment only if you have benchmarked it on this specific setup
        ...
        for epoch in range(EPOCHS):
            ...
            for input_ids, attention_mask, y in tqdm(
                ...
                # FIX 3: explicitly delete outputs to free GPU memory on long runs
                del outputs
