#!/usr/bin/env python3
"""
Compute SVCCA scores (diagonal + full matrix) for encoder models.
"""
...
from Similarity_utils import (
    ...
    compute_svcca_diagonal,
    compute_svcca_full_matrix,
    save_results
)
...
REP_ROOT = "/rep_root_folder"
OUTPUT_DIR = "/output_dir/svcca_results"
...
for model_name in MODELS:
    ...
    for lang1, lang2 in LANGUAGE_PAIRS:
        ...
        # Compute SVCCA
        diagonal = compute_svcca_diagonal(reps1, reps2, k=K_COMPONENTS, device=DEVICE)
        full_matrix = compute_svcca_full_matrix(reps1, reps2, k=K_COMPONENTS, device=DEVICE)
        ...
        # Plot
        plot_diagonal(diagonal, f"{model_name} {pair_name} Diagonal SVCCA", 
                     os.path.join(output_dir, f"{pair_name}_diagonal"))
        plot_heatmap(full_matrix, f"{model_name} {pair_name} SVCCA Matrix",
